...
def create_parser():
    ...
    parser.add_argument('--binary_file', type=str,
                        default="data/2016-MMseqs/binary_dir/PredGO_O82330.pt")
...
if __name__ == '__main__':
    ...
    score = model.predict_from_binary_file(args.binary_file, args.device)
    term_score = {}
    for i, s in enumerate(score[0]):
        term_score[index2term[i]] = s
    ...
    with open(os.path.join(args.out_dir, f'{os.path.basename(args.binary_file)}_go_score.json'), 'w') as f:
